...
function convert(x){
 ...
 draw();
		...
		else if (!dataa1.length==0&&dataa2.length==0)
			{
			var date = $(this).children('td:eq(2)').text(), distance = $(this).children('td:eq(0)').text(),time = $(this).children('td:eq(1)').text();
			globaltime2 = time;
			globaldate2 = date;
    $.ajax({
        ...
        success: function(datta){
			...
			dataa2.push({x:globaldate2,y:globaltime2});
    ...
    if (Date.parse(dataa2[i].x) > Date.parse(globaldate2)) {
        pointBackgroundColors2.push("#0e918a");
		pointBorderColors2.push("#0e918a");
    } else if (Date.parse(dataa2[i].x) < Date.parse(globaldate2)) {
        pointBackgroundColors2.push("#79ffd2");
		pointBorderColors2.push("#79ffd2");
    }
								else if (Date.parse(dataa2[i].x)==Date.parse(globaldate2))
									{
										pointBackgroundColors2.push("#00faff");
		                    pointBorderColors2.push("#00faff");
									}
...
<script>
	$(document).mouseup(function (e){ 
        var block = $("#mySidenav"); 
        if (!block.is(e.target)&& block.has(e.target).length === 0) { // && block.has(e.target).length === 0 проверка условия если клик не по его дочерним элементам
